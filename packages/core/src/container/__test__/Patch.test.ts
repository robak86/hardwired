...
describe(`Patch`, () => {
  ...
  it(`works as global override`, async () => {
    ...
    const use = container(otherOverrides); // override globally and prevent from being overridden in child scopes
    ...
    expect(response1.value).toEqual(response2.value);
    expect(response1.db).toEqual(response1.nested.db);
    expect(response1.db).toEqual('db');
    expect(response1.nested.db).toEqual('db');
    expect(response1.value).toEqual(response1.nested.value);

    expect(response2.value).toEqual(response2.nested.value);
    expect(response2.nested.db).toEqual('db');
    expect(response2.db).toEqual('db');
    expect(response2.nested.db).toEqual('db');
    expect(response2.value).toEqual(response2.nested.value);
  });
});
