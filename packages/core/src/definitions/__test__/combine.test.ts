...
describe(`combine`, () => {
  ...
  describe(`combine without args`, () => {
    it(`returns correct instance`, async () => {
      ...
      expect(root.use(defWithMiddleware)).toBe(123);
    });
  });
  ...
  describe(`passthrough`, () => {
    ...
    it(`works with args`, async () => {
      ...
      expect(root.use(withMiddleware, 123, '123')).toEqual([123, '123']);
      expect(root.use(withoutMiddleware, 123, '123')).toEqual([123, '123']);
    });
  });
  ...
  describe(`creatingScope`, () => {
    it(`returns correct instance`, async () => {
      ...
      const req1Value = root.use(defWithMiddleware);
      const req2Value = root.use(defWithMiddleware);
