...
describe(`class`, () => {
  ...
  const syncConfig = configureContainer(c => {
    ...
    c.add(myClassSingleton).fn((num, str) => new MyClass(num, str), numDef, strDef);
    c.add(myClassScoped).fn((num, str) => new MyClass(num, str), numDef, strDef);
    c.add(myClassCascading).fn((num, str) => new MyClass(num, str), numDef, strDef);
  });
