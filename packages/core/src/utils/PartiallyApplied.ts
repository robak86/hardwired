...
export type PartialFnDependencies<TArgs, TLifeTime extends LifeTime> =
    [] extends TArgs ? []:
    TArgs extends [...infer TPRev, any] ? ( {[K in keyof TArgs]: AsyncInstanceDefinitionDependency<TArgs[K],TLifeTime>} ) | PartialFnDependencies<TPRev, TLifeTime> : [];

// prettier-ignore
export type AsyncPartialFnDependencies<TArgs extends any[], TLifeTime extends LifeTime> =
    [] extends TArgs ? []:
        TArgs extends [...infer TPRev, any] ? ( {[K in keyof TArgs]: InstanceDefinitionDependency<TArgs[K],TLifeTime>} ) | AsyncPartialFnDependencies<TPRev, TLifeTime> : [];
