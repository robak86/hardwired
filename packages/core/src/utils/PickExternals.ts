...
type ExtractExternals<T> = T extends { externals: infer TExternals }
  ? { [K in keyof TExternals]: TExternals[K] extends InstanceDefinition<infer TValue, any, any> ? TValue : never }
  : never;
...
export const pickExternals = <T extends WithExternals<any>[]>(externals: T): PickExternals<T> => {
